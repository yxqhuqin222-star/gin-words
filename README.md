...
- **前端**：根目录 `index.html` 可直接打开；发音使用随项目发布的固定英文 MP3，无法加载时回退到浏览器 Web Speech API。
- **两种学习模式**：顶部分别「单词卡 / 常用句」入口，进度各自独立记录。常用句按场景分好中英对照，共 212 句 / 10 个场景（机场海关、酒店、咖啡店、餐厅、购物退税、交通问路、瑞士、闲聊、紧急情况、万能句）。
- **发音**：固定音频使用英式英语 AI 神经网络音色生成，网站运行时不调用付费 API；音频文件位于 `assets/audio/`。
